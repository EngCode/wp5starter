 ...
 *
 * @link http://embed.ly/docs/embed/api/endpoints/1/oembed
 * @link http://embed.ly/embed/features/providers
 *
...
if ( ! class_exists( 'WPSEO_Video_Details_Embedly' ) ) {
	...
	class WPSEO_Video_Details_Embedly extends WPSEO_Video_Details_Oembed {
		...
		/**
		 * Embedly API key from saved options.
		 *
		 * @static
		 *
		 * @var string
		...
		/**
		 * Whether or not we're still getting a response when retrieval is done without API key.
		...
		public function __construct( $vid, $old_vid = array() ) {
			if ( empty( $this->api_key ) ) {
				...
				$embedly_api_key = WPSEO_Options::get( 'video_embedly_api_key', '' );
				if ( $embedly_api_key !== '' ) {
					$this->api_key = $embedly_api_key;
				}
			}
			if ( ! empty( $this->api_key ) ) {
