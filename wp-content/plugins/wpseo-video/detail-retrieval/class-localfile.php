...
if ( ! class_exists( 'WPSEO_Video_Details_Localfile' ) ) {
	 ...
	 *
	 *            This also means that this class uses a few extra $vid keys to a) pass things to this
	 *            class and b) retain the information found.
	 *
	 *            The 'maybe_local' (bool) key is used to determine whether to call this class. It would
	 *            be nicer to use 'type' = 'localfile' for this to be in line with the other detail
	 *            retrieval classes, but that could break existing filters on types 'jwplayer',
	 *            'mediaelementjs' etc which have been used up to now for local files from various sources.
	 *
	 ...
	 *
	 *            The local property $file_url always *has* to be set, of the local properties $file_path
	 *            and $attachment_id only one or the other is expected.}
	 */
	class WPSEO_Video_Details_Localfile extends WPSEO_Video_Details {
			...
			if ( strpos( $url, 'http' ) === 0 || strpos( $url, '//' ) === 0 ) {
				...
				if ( strpos( $url, $site_url ) === 0 || ( ! empty( $network_url ) && strpos( $url, $network_url ) === 0 ) ) {
					...
					if ( $parsed_url['file'] !== '' ) {
						...
						elseif ( $ext === false ) {
							/*
							 * {@internal At some point in the future we may want to switch this over
							 *            to the attachment_url_to_postid( $url ) function which is
							 *            introduced in WP 4.0.}
							 */
		 ...
		 *            The value will in that case be a string, which is why we use
		 *            length_formatted in that case.
		 *
