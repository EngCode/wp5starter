 ...
 *
 * @link http://wistia.com/doc/data-api For the API (but needs a API key, while oEmbed
 *                                      actually already gives us what we need).
 *
...
if ( ! class_exists( 'WPSEO_Video_Details_Wistia' ) ) {
	...
	class WPSEO_Video_Details_Wistia extends WPSEO_Video_Details {
		...
		/**
		 * Whether or not to use the fallback method to retrieve certain information on the video.
		 *
		 * @var bool
		 */
		...
		/**
		 * Frame source - used by fallback method.
		 *
		 * @var string
		 */
		...
		protected function set_player_loc() {
			...
			elseif ( ! empty( $this->frame_source ) ) {
				...
				if ( isset( $flv, $still, $account_key, $media_key, $this->vid['duration'] ) ) {
					$url = sprintf(
						'https://wistia.sslcs.cdngc.net/flash/embed_player_v2.0.swf?videoUrl=%1$s&stillUrl=%2$s&controlsVisibleOnLoad=false&unbufferedSeek=true&autoLoad=false&autoPlay=true&endVideoBehavior=default&embedServiceURL=http://distillery.wistia.com/x&accountKey=%3$s&mediaID=%4$s&mediaDuration=%5$s&fullscreenDisabled=false',
						$flv, // #1.
						$still, // #2.
						$account_key, // #3.
						$media_key, // #4.
