	...
	class WPSEO_Local_Admin_Map_Settings {
		...
		public function maps_settings() {
			...
			if ( ! empty( $api_key_browser ) || ! empty( $api_key ) ) {
				...
				echo '<img src="' . ( isset( $custom_marker ) ? esc_url( wp_get_attachment_url( $custom_marker ) ) : '' ) . '" id="local_custom_marker_image_container" class="wpseo-local-hide-button' . ( ( false === $show_marker ) ? ' hidden' : '' ) . '">';
				...
				echo '<br>';
				echo '<a href="javascript:" class="remove_custom_image wpseo-local-hide-button' . ( ( false === $show_marker ) ? ' hidden' : '' ) . '" data-id="local_custom_marker">' . esc_html__( 'Remove marker', 'yoast-local-seo' ) . '</a>';
				echo '</div>';
				echo '</div>';
				if ( true === $show_marker ) {
