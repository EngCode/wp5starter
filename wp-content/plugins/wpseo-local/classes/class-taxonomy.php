...
if ( ! class_exists( 'WPSEO_Local_Taxonomy' ) ) {
	...
	class WPSEO_Local_Taxonomy {
		...
		public function term_seo_form( $term ) {
			...
			global $wpseo_taxonomy, $wpseo_local_core;
			...
			echo '<img src="' . esc_url( wp_get_attachment_url( $tax_meta['wpseo_local_custom_marker'] ) ) . '" id="custom_marker_image_container" class="wpseo-local-hide-button' . ( ( false === $show_marker ) ? ' hidden' : '' ) . '">';
			...
			echo ' <a href="javascript:;" id="remove_marker" class="remove_custom_image wpseo-local-hide-button' . ( ( false === $show_marker ) ? ' hidden' : '' ) . '" data-id="custom_marker" style="color: #a00;">' . esc_html__( 'Remove marker', 'yoast-local-seo' ) . '</a>';
