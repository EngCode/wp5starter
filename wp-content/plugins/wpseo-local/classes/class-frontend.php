...
if ( ! class_exists( 'WPSEO_Local_Frontend' ) ) {
	...
	class WPSEO_Local_Frontend {
		...
		public function __construct() {
			...
			add_action( 'wpseo_head', array( $this, 'maybe_show_geo_meta' ), 90 );
		}
		...
		public function maybe_show_geo_meta() {
			if ( ! wpseo_has_multiple_locations() || is_singular( 'wpseo_locations' ) ) {
				...
				if ( ! empty( $location['business_city'] ) ) {
					echo '<meta name="geo.placename" content="' . esc_attr( $location['business_city'] ) . '" />' . "\n";
				}
				if ( ! empty( $location['coords']['lat'] ) && ! empty( $location['coords']['long'] ) ) {
					echo '<meta name="geo.position" content="' . esc_attr( $location['coords']['lat'] . ';' . $location['coords']['long'] ) . '" />' . "\n";
				}
				if ( ! empty( $location['business_country'] ) ) {
					echo '<meta name="geo.region" content="' . esc_attr( $location['business_country'] ) . '" />' . "\n";
				}
			}
		}
	}
