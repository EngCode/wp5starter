const{__}=wp.i18n;const{registerBlockType}=wp.blocks;const{withInstanceId}=wp.compose;const{PlainText,InspectorControls,InnerBlocks,RichText,Editable,MediaUpload,MediaUploadCheck,AlignmentToolbar,BlockControls,ColorPalette}=wp.editor;const{Draggable,Panel,PanelBody,PanelRow,SelectControl,Button,DropdownMenu,withAPIData,TextControl,ServerSideRender,RangeControl,FormFileUpload,BaseControl}=wp.components;const{Component,Fragment}=wp.element;const{withSelect,dispatch}=wp.data;const{createHigherOrderComponent}=wp.compose;
