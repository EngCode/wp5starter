const{__}=wp.i18n;const{registerBlockType}=wp.blocks;const{withInstanceId}=wp.compose;if(!wp.editor){const{PlainText,RichText,Editable,MediaUpload,MediaUploadCheck,AlignmentToolbar,BlockControls,ColorPalette,InnerBlocks,InspectorControls}=wp.blockEditor}else{const{PlainText,RichText,Editable,MediaUpload,MediaUploadCheck,AlignmentToolbar,BlockControls,ColorPalette,InnerBlocks,InspectorControls}=wp.editor}const{Draggable,Panel,PanelBody,PanelRow,SelectControl,Button,DropdownMenu,withAPIData,TextControl,RangeControl,FormFileUpload,BaseControl}=wp.components;if(!wp.components.serverSideRender){const ServerSideRender=wp.components.serverSideRender}else{const ServerSideRender=wp.serverSideRender}const{Component,Fragment}=wp.element;const{select,withSelect,dispatch}=wp.data;const{createHigherOrderComponent}=wp.compose;
